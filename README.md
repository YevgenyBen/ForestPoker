...
### WhatsApp settlements (optional)

When a game is closed, the app can send you a WhatsApp message with the settlement transfers via [Twilio WhatsApp](https://www.twilio.com/docs/whatsapp).

1. Twilio Console → **Messaging** → **Try it out** → **Send a WhatsApp message** (sandbox) or configure an approved WhatsApp sender.
2. If using the sandbox, send the join code from the console to the sandbox number from your phone so Twilio can deliver to you.
3. In `.env.local` (and App Hosting secrets in production), set all four variables from `.env.example`: `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_WHATSAPP_FROM`, `WHATSAPP_NOTIFY_TO` (both numbers as `whatsapp:+E164`).
4. Close a balanced game — you should receive the settlement summary. If Twilio is not configured or the send fails, the game still closes successfully.
